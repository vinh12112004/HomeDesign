...
const TexturePicker = ({ value, onChange }) => {
    ...
    useEffect(() => {
        if (open && items.length === 0)
            dispatch(fetchAssets({ type: "texture" }));
    ...
    const filtered = items.filter(
        (url) => !filter || url.toLowerCase().includes(filter.toLowerCase())
    );
        ...
        <div>
            <Space>
                {enabled && value ? (
                    ...
                    <div
                        style={{ width: 64, height: 64, background: "#eee" }}
                    />
                )}
